...
namespace PlacesApp.Utils
{
    ...
    {
        static string url = "http://192.168.1.10/WSGOPLAY/api/Canchas";
        ...
        public static async Task<ObservableCollection<Canchas>> GetCanchasAsync()
        {
            var respuesta = new ObservableCollection<Canchas>();
            var http = new HttpClient();
            var get = await http.GetAsync(url);
            if (get.IsSuccessStatusCode)
            {
                var contenido = await get.Content.ReadAsStringAsync();
                respuesta = JsonConvert.DeserializeObject<ObservableCollection<Canchas>>(contenido);
            }
            return respuesta;
        }
    }
